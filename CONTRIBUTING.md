...
| `gallery` | `list[object]` | Ordered local images and YouTube videos, each with `type` and `caption` |
...
- Gallery entries use supported media types, project-scoped image paths or YouTube video IDs, and valid captions
...
Gallery media is optional and appears in the same order as the YAML entries. Images must be hosted locally in this repository; YouTube videos are referenced by video ID and load in a privacy-enhanced player after a visitor presses play.

### Adding Gallery Media

1. For images, create a subdirectory under `images/screenshots/<your-project-id>/`
2. Add between 1 and 10 total image and YouTube items
3. Give every item a short caption and place the strongest media first:
...
For a YouTube URL such as `https://www.youtube.com/watch?v=M7lc1UVf-VE`, submit only the `M7lc1UVf-VE` video ID.

### Requirements

- **Types**: `image` or `youtube`
- **Image path format**: `images/screenshots/<project-id>/<filename>`
- **Image formats**: lowercase `.png`, `.jpg`, `.jpeg`, or `.webp`
- **Image file size**: 2 MB maximum per image
- **YouTube ID**: exactly 11 letters, numbers, hyphens, or underscores; full URLs and playlist IDs are not accepted
- **Count**: 1–10 gallery items total; 3–6 is a good range for most projects
